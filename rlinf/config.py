...
def validate_embodied_cfg(cfg):
    ...
    if cfg.rollout.get("backend", "huggingface") == "offline":
        return cfg
    ...
    with open_dict(cfg):
        if cfg.env.train.simulator_type == "maniskill":
            ...
            for candidate in (
                "r1pro_behavior_from_env_cfg.yaml",
                "r1pro_behavior_from_env_cfg_simple.yaml",
                "r1pro_behavior.yaml",
            ):
                path = os.path.join(og.example_config_path, candidate)
                if os.path.isfile(path):
                    config_filename = path
                    break
            if config_filename is None:
                config_filename = os.path.join(
                    og.example_config_path, "r1pro_behavior_from_env_cfg_simple.yaml"
                )
