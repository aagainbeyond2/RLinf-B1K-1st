...
class EnvOutput:
    ...
    def prepare_observations(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        task_id = obs["task_id"] if "task_id" in obs else None
        tokenized_prompt = obs["tokenized_prompt"] if "tokenized_prompt" in obs else None
        tokenized_prompt_mask = (
            obs["tokenized_prompt_mask"] if "tokenized_prompt_mask" in obs else None
        )
...
class ChunkStepResult:
    ...
    def __post_init__(self):
        ...
        if self.episode is not None:
            self.episode = put_tensor_cpu(self.episode)
...
class EmbodiedRolloutResult:
    ...
    def append_result(self, result: ChunkStepResult):
        ...
        if result.episode is not None:
            self.episode.append(result.episode)
        elif len(self.episode) > 0:
            self.episode.append({})

    def episode_to_dict(self) -> Optional[dict[str, torch.Tensor]]:
        if len(self.episode) == 0:
            return None

        steps = len(self.episode)
        per_key: dict[str, list[Optional[torch.Tensor]]] = {}
        for i, step_episode in enumerate(self.episode):
            if not isinstance(step_episode, dict):
                continue
            for k, v in step_episode.items():
                if not isinstance(v, torch.Tensor):
                    continue
                if k not in per_key:
                    per_key[k] = [None] * steps
                per_key[k][i] = v.cpu().contiguous()
        ...
        for k, seq in per_key.items():
            template = next((x for x in seq if isinstance(x, torch.Tensor)), None)
            if template is None:
                continue
            filled = [
                (x if x is not None else torch.zeros_like(template)).cpu().contiguous()
                for x in seq
            ]
            out[k] = torch.stack(filled, dim=0).cpu().contiguous()
    ...
    def to_splited_dict(self, split_size) -> list[dict[str, Any]]:
        ...
        debug_shapes = os.environ.get("RLINF_DEBUG_SHAPES", "0") == "1"
        ...
        if debug_shapes:
            ...
            print(
                f"[RLinf][debug_shapes] to_splited_dict split_size={split_size} tensor_shapes={tensor_shapes}",
                flush=True,
            )

        rollout_result_list: list[dict[str, Any]] = []
        for i in range(split_size):
            split_dict: dict[str, Any] = {}
            for key, value in base_dict.items():
                if isinstance(value, torch.Tensor):
                    if value.size(1) < split_size:
                        raise ValueError(
                            f"Cannot split rollout tensor '{key}' with batch_dim={value.size(1)} "
                            f"into split_size={split_size}. tensor_shape={tuple(value.shape)}"
                        )
                    chunks = torch.chunk(value, split_size, dim=1)
                    if i >= len(chunks):
                        raise ValueError(
                            f"torch.chunk returned {len(chunks)} chunks < split_size={split_size} "
                            f"for tensor '{key}' with shape={tuple(value.shape)}"
                        )
                    split_dict[key] = chunks[i].contiguous()
                else:
                    split_dict[key] = value
