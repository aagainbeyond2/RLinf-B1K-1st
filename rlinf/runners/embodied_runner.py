...
class EmbodiedRunner:
    ...
    def update_rollout_weights(self):
        if self.actor is None or self.cfg.runner.get("rollout_only", False):
            return
        if self.cfg.rollout.get("backend", "huggingface") == "offline":
            return
        rollout_futures = self.rollout.sync_model_from_actor()
        actor_futures = self.actor.sync_model_to_rollout()
        actor_futures.wait()
        if rollout_futures is not None:
    ...
    def generate_rollouts(self):
        if self.cfg.runner.get("rollout_only", False):
            ...
            return compute_evaluate_metrics(env_results_list)
        if self.cfg.rollout.get("backend", "huggingface") == "offline":
            rollout_futures = self.rollout.generate()
    ...
    def run(self):
        ...
        for _step in range(start_step, self.max_steps):
            ...
            with self.timer("step"):
                ...
                if self.actor is not None and not self.cfg.runner.get("rollout_only", False):
                ...
                if save_model and self.actor is not None and not self.cfg.runner.get("rollout_only", False):
                    self._save_checkpoint()

            time_metrics = self.timer.consume_durations()

            time_metrics = {f"time/{k}": v for k, v in time_metrics.items()}
            rollout_metrics = {f"rollout/{k}": v for k, v in actor_rollout_metrics[0].items()}
            env_metrics = {f"env/{k}": v for k, v in env_metrics.items()}
            time_metrics = {f"time/{k}": v for k, v in time_metrics.items()}
            training_metrics = {f"train/{k}": v for k, v in actor_training_metrics[0].items()}
