...
_WARNED_DISABLE_TYPECHECK = False
...
class Observation(Generic[ArrayT]):
    ...
    def from_dict(cls, data: at.PyTree[ArrayT]) -> "Observation[ArrayT]":
        ...
        for key in data["image"]:
            if isinstance(data["image"][key], torch.Tensor) and data["image"][key].ndim == 3:
                data["image"][key] = data["image"][key].unsqueeze(0)
        ...
        if should_disable_typecheck:
            global _WARNED_DISABLE_TYPECHECK
            if not _WARNED_DISABLE_TYPECHECK:
                logger.warning(
                    "Observation.from_dict received torch.Tensor inputs; disabling runtime typechecking."
                )
                _WARNED_DISABLE_TYPECHECK = True
