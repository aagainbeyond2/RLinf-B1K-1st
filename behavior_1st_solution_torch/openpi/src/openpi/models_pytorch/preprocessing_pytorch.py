...
def preprocess_observation_pytorch(
    ...
    *,
    ...
    first_image = observation.images[image_keys[0]]
    if first_image.ndim not in (4, 5):
        raise ValueError(
            f"Unsupported image ndim={first_image.ndim}. Expected 4D or 5D tensors."
        )
    batch_shape = first_image.shape[: first_image.ndim - 3]
    ...
    for key in image_keys:
        ...
        if image.ndim not in (4, 5):
            raise ValueError(
                f"Unsupported image ndim={image.ndim} for key={key}. Expected 4D or 5D tensors."
            )

        is_video = image.ndim == 5
        if is_video:
            is_channels_first = image.shape[2] == 3
        else:
        ...
        if is_video:
            ...
            image = image.reshape(b * s, *image.shape[2:])
        else:
            if is_channels_first:
        ...
        if is_video:
            image = image.reshape(b, s, *image.shape[1:])
            if is_channels_first:
    ...
    for key in out_images:
        if key not in observation.image_masks:
            image = out_images[key]
            out_masks[key] = torch.ones(
                image.shape[: image.ndim - 3],
                dtype=torch.bool,
