...
def load_and_infer(
    ...
    checkpoint_dir="/data1/heyelin/models/behavior_1st/checkpoint_2/params/", # 如果config里有，可以填None；否则手动指定
    ...
    def init_model_fn(rng):
        ...
        if not getattr(builtins, 'DEBUG_FALG', False):
            # 注释使用 fp32 推理
            # [修复开始] ------------------------------------------------
    ...
    # DEBUG_FLAG 开关
    # builtins.DEBUG_FALG = True
    builtins.DEBUG_FALG = False
    print(f"builtins.DEBUG_FALG is {builtins.DEBUG_FALG}")
    builtins.ENABLE_SIGLIP_DEBUG = False

    if getattr(builtins, 'DEBUG_FALG', False):
        # debug for eager mode
        train_state_shape = init_model_fn(init_rng)
    else:
    ...
    try:
        ...
        # # ================== [插入 Debug 代码] ==================
        # print("\n=== DEBUG: Parameter Key Mismatch Check ===")
        #
        # # 1. 获取模型预期的 Key (Destination)
        # # 将嵌套字典展平，只看前几层路径
        # def get_flat_keys(d, prefix=""):
        #     keys = []
        #     for k, v in d.items():
        #         new_key = f"{prefix}.{k}" if prefix else k
        #         if isinstance(v, dict): # 或者是 nnx.State/Variable
        #             # 如果是纯字典，继续递归；如果是叶子节点则停止
        #             # 注意：target_params_dict 是纯字典
        #             if 'kernel' in v or 'scale' in v: # 到了参数层
        #                 keys.append(new_key)
        #             else:
        #                 keys.extend(get_flat_keys(v, new_key))
        #         else:
        #              keys.append(new_key)
        #     return keys
        #
        # # 打印模型里真正需要的 Key (前 10 个)
        # model_keys = get_flat_keys(target_params_dict)
        # print(f"Model expects keys (Example): {model_keys[:5]}")
        #
        # # 2. 获取加载到的 Key (Source)
        # ckpt_keys = get_flat_keys(loaded_params_dict)
        # print(f"Checkpoint has keys (Example): {ckpt_keys[:5]}")
        #
        # # 3. 检查关键路径 (Transformer/LayerNorm)
        # print(f"\nChecking specific path 'Transformer'...")
        # has_trans_model = any("Transformer" in k for k in model_keys)
        # has_trans_ckpt = any("Transformer" in k for k in ckpt_keys)
        # print(f"  - Exists in Model? {has_trans_model}")
        # print(f"  - Exists in Checkpoint? {has_trans_ckpt}")
        #
        # # 4. 打印不匹配的前缀
        # # 比如模型是 'PaliGemma.img.Transformer...' 而 Checkpoint 是 'vision_tower.Transformer...'
        # if model_keys and ckpt_keys:
        #     print(f"  - Model Top Key: {model_keys[0].split('.')[0]}")
        #     print(f"  - Ckpt  Top Key: {ckpt_keys[0].split('.')[0]}")
        # # =======================================================
    ...
    if getattr(builtins, 'DEBUG_FALG', False):
        # debug for eager mode
        print("DEBUG mode : JIT disabled. Running eagerly.")
    else:
        # [修复]: 使用 @nnx.jit 替代 @jax.jit
        forward_fn = nnx.jit(forward_fn)
...
if __name__ == "__main__":
    # 运行
    jax.config.update("jax_disable_jit", True)  # 全局禁用jit
    load_and_infer()