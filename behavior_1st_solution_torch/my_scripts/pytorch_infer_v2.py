...
class ObsAdapter:
    ...
    def __init__(self, jax_obs, device):
        self.images = {}
        self.image_masks = {}
        float_dtype = torch.bfloat16 if device == "cuda" else torch.float32
        ...
        for k, v in jax_obs.images.items():
            ...
            img_pt = torch.from_numpy(img_np).permute(0, 1, 4, 2, 3).to(device=device, dtype=float_dtype)
            self.images[k] = img_pt
        ...
        # --- 3. 处理状态和 Prompt ---
        self.state = torch.from_numpy(np.array(jax_obs.state)).to(device=device, dtype=float_dtype)
...
def main():
    ...
    pt_actions = torch.from_numpy(np.array(jax_actions)).to(device=device, dtype=float_dtype)
    ...
    with open(os.path.join(REPRODUCE_DIR, "batch_noise_time.pkl"), "rb") as f:
        ...
        fixed_noise = torch.from_numpy(batch_noise_time['noise']).to(device=device, dtype=float_dtype)
        fixed_time = torch.from_numpy(batch_noise_time['time']).to(device=device, dtype=float_dtype)
    ...
    state_dict = torch.load(os.path.join(REPRODUCE_DIR, "final_b1k_pytorch.pt"), map_location="cpu")
