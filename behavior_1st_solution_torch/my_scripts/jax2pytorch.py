...
def convert_and_save_weights(pytorch_model, jax_params_flat, output_path=None):
    """
    ...
    for pt_key, pt_tensor in pt_state_dict.items():
        ...
        #
        # if pt_key != "paligemma_with_expert.paligemma.model.language_model.layers.0.self_attn.k_proj.weight":
        #     continue
                ...
                if "self_attn" in pt_key:
                    ...
                    if "q_proj.weight" in pt_key:
                        ...
                        if jax_k in jax_params_flat:
                            ...
                            jax_data = raw_data.reshape(-1, raw_data.shape[-1])
                        ...
                        if jax_k in jax_params_flat:
                            ...
                            full_kv_tensor = torch.from_numpy(np.array(full_kv))
                            ...
                            if "k_proj" in pt_key:
                                jax_data = full_kv_tensor[0]
                            else:
                                jax_data = full_kv_tensor[1]
                            jax_data = jax_data.permute(0, 2, 1)
                            jax_data = jax_data.reshape(-1, jax_data.shape[-1])
...
if __name__ == "__main__":
    ...
    # 用于对齐
    INPUT_PATH = "/data1/heyelin/models/behavior_1st_torch/checkpoint_2/random_state.pth"
    # ===========================================
    ...
    # 3. 运行
    convert_and_save_weights(model, jax_params_flat, output_path=os.path.join(REPRODUCE_DIR, 'final_b1k_pytorch.pt'))
    pass