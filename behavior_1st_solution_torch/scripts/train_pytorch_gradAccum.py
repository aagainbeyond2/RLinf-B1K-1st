...
class LazyBehaviorDataset(Dataset):
    """
    具备【延迟加载】和【定期自清理】功能的代理 Dataset。
    
    1. 主进程只持配置，不占内存。
    2. Worker 进程按需加载。
    3. Worker 进程每读取一定数量样本后，强制销毁 Dataset 并 GC，彻底杜绝缓存堆积。
    """
    def __init__(self, data_config, action_horizon, seed, reset_interval=20000):
        """
        Args:
            reset_interval: 每被访问多少次后触发一次重置。
                            假设 batch_size=64, num_workers=4, 
                            每个 worker 每跑 100 个 step 大约会处理 1600 个样本。
                            设置 20000 大约意味着每 1000-1500 个 Step 重置一次。
                            设置太小会影响速度，设置太大内存会溢出。
        """
        ...
        # # [主进程] 预先计算长度
        # # 这需要一次临时的加载，但加载完立即销毁，不会占用主进程长驻内存
        # print(f"[LazyDataset] Pre-calculating dataset length in Main Process...")
        # temp_dataset = _b1k_loader.create_behavior_dataset(
        #     data_config,
        ...
        # del temp_dataset
        gc.collect()
        print(f"[LazyDataset] Length calculated: {self._len}. Main process memory cleared.")
    ...
    def __getitem__(self, idx):
        ...
        # =========================================================
        # [核心逻辑] 定期清理内存 (Suicide & Respawn)
        # =========================================================
        if self.inner_dataset is not None and (self.access_count % self.reset_interval == 0):
            # 获取当前进程 ID，方便调试
            pid = os.getpid()
            # print(f"[Worker {pid}] Hit {self.access_count} access. Resetting dataset to clear cache...")
            
            # 1. 彻底删除引用
            del self.inner_dataset
            self.inner_dataset = None
            
            # 2. 强制垃圾回收 (清除 Numpy/C++ 层的碎片)
            gc.collect()
            
            # print(f"[Worker {pid}] Dataset reset complete. Memory freed.")
        ...
        if self.inner_dataset is None:
            # pid = os.getpid()
            # print(f"[Worker {pid}] Initializing Inner Dataset...")
            
            # 1. 创建 Base Dataset
            base_ds = _b1k_loader.create_behavior_dataset(
                self.data_config, 
                self.action_horizon, 
                self.seed
            )
            
            # 2. 应用 Transforms (Normalize 等)
            self.inner_dataset = _b1k_loader.transform_dataset(
                base_ds, 
                self.data_config, 
...
def worker_init_fn(worker_id):
    ...
    Worker 初始化函数：
    1. 设置随机种子
    2. [关键] 限制 Numpy 线程数，防止内存和 CPU 爆炸
    """
    worker_seed = torch.initial_seed() % 2**32
    np.random.seed(worker_seed)
    random.seed(worker_seed)
    
    # 强制 Numpy 在 Worker 内部只用 1 个线程
    # 这能显著降低每个 Worker 的内存开销
...
def build_b1k_dataloader(config, rank, world_size):
    ...
    loader = DataLoader(
        ...
        sampler=sampler,
        num_workers=config.num_workers,        # 建议先开 4 个
        prefetch_factor=2,    # 限制预取
